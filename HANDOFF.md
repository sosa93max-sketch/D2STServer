...
- **Last update:** the server was retargeted from Dota 7.22g to the **current
  build (ClientVersion 6783, May 2026)**: protobufs regenerated from
...
- Current `steam.inf`: `ClientVersion=6783`, `ServerVersion=6783`,
  `SourceRevision=10633231`, `VersionDate=May 04 2026`, `appID=570`.
- Legacy 7.22g (`ClientVersion=3756`, Sep 2019) is kept as a secondary profile.
...
5. **Version profiles** — `VersionProfiles.Resolve(clientVersion)` returns
   `V6783` for `clientVersion >= 6783`, `V722g` for `[1, 3756]`, else `Modern`.
...
7. **Full protobuf contract set** — generated into
   ...
   `tools/regenerate-protos.sh` (pinned to GameTracking-Dota2 `4b28dd7d49`,
   the build-6783 commit of 2026-05-04). The 2026 protos added new game-mode
   contracts and changed the lobby SO (`AllMembers` + member indices instead of
   the old `members` list; members carry no name — names live in
   `LobbyService._memberNames`).
...
21. **Chat (stage 4h)** — `ChatService` keeps the chat channels. A channel is
    *not* a Shared Object: the client holds no cache of one and reacts only to
    ...
    belongs to its group and the client opens it itself) and member count
    ...
    8091 — an admin cannot be kicked and the last
...
- Stage 4h over HTTP with two logged-on players (`tools/verify-chat.py`, 37
  ...
  nothing; 8048 reports both members; leaving pushes 7014, kills a
...
- Stage 4g launch over HTTP (`tools/verify-lobby.py`, now 45 checks, hello
  version 6783): 7041
...
10. **steam_api interface coverage for the current build.** With the real client (installed
    as described in §4), capture `Not found Interface for ...` logs and ensure the shim exposes the interface
    versions the new client requests (the interface versions 7.22g used are
    obsolete).
...
### How to validate the current build end-to-end (must be on Windows with the real client)
```
...
client (build 6783) + a `steam_api` shim that supports its interface versions.
